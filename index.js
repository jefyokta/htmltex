...
var xmlDecodeTree = /* @__PURE__ */ new Uint16Array(/* @__PURE__ */ "Ȁaglq\t\x15\x18\x1Bɭ\x0F\x00\x00\x12p;䀦os;䀧t;䀾t;䀼uot;䀢".split("").map((c) => c.charCodeAt(0)));
...
`;
  ...
  texToHtml: {
    ...
    chapter: ({ content }) => `<h1 >${content}</h1>`,
    section: ({ content }) => `<h2 >${content}</h2>`,
    subsection: ({ content }) => `<h3>${content}</h3>`,
    subsubsection: ({ content }) => `<h4>${content}</h4>`,
    ...
    par: (data, content, text) => `<p>${text}</p>`,
    textcolor: (attrs, content) => `<span style="color:${attrs[1]}">${content}</span>`,
    begin_itemize: () => `<ul>`,
    end_itemize: () => `</ul>`,
    begin_enumerate: () => `<ol>`,
    end_enumerate: () => `</ol>`,
    item: (attrs, content, text) => `<li>${convertLatexToHtml(text)}</li>`,
    includegraphics: (attrs) => `<img src="${attrs[1]}" style="width:${attrs[2] || "50%"}" />`,
    href: (attrs, content) => `<a href="${attrs[1]}">${content}</a>`,
    rule: () => `<hr />`,
...
var convertHtmlToLatex = (html) => {
  ...
  return document2.map(convertNode).join("");
};
var convertLatexToHtml = (latex) => {
  ...
  return latex.replace(tablePattern, (match, content) => {
    ...
    return `<figure ${centered ? CenteredLabeledImage : ""} ${label ? `id="${label[1]}"` : ""}>
        ...
        `;
  }).replaceAll(latexPattern, (match, command2, content, text, ...f) => {
    const converter = LatexConverter.texToHtml[command2];
    if (converter) {
      const attrs = {
        color: content || "",
        src: content || "",
...
var LabeledImage = Node2.create({
  ...
  renderHTML({ node: node2, HTMLAttributes }) {
    ...
    const { src, width, caption, centered, cite } = node2.attrs;
    return [
      "figure",
...
var latex = convertHtmlToLatex(html);
console.log(latex);
