...
function doWrapInList(tr, range, wrappers, joinBefore, listType) {
  ...
  for (let { startIndex: i, endIndex: e } = range, first = true;i < e; i++, first = false) {
...
var focus = (position = null, options = {}) => ({ editor, view, tr, dispatch }) => {
  ...
  const delayedFocus = () => {
    ...
    requestAnimationFrame(() => {
      if (!editor.isDestroyed) {
        view.focus();
        if (options === null || options === undefined ? undefined : options.scrollIntoView) {
...
var StarterKit = Extension.create({
  ...
  addExtensions() {
    ...
    if (this.options.bold !== false) {
      extensions.push(Bold.configure((_a = this.options) === null || _a === undefined ? undefined : _a.bold));
    }
    if (this.options.blockquote !== false) {
      extensions.push(Blockquote.configure((_b = this.options) === null || _b === undefined ? undefined : _b.blockquote));
    }
    if (this.options.bulletList !== false) {
      extensions.push(BulletList.configure((_c = this.options) === null || _c === undefined ? undefined : _c.bulletList));
    }
    if (this.options.code !== false) {
      extensions.push(Code.configure((_d = this.options) === null || _d === undefined ? undefined : _d.code));
    }
    if (this.options.codeBlock !== false) {
      extensions.push(CodeBlock.configure((_e = this.options) === null || _e === undefined ? undefined : _e.codeBlock));
    }
    if (this.options.document !== false) {
      extensions.push(Document.configure((_f = this.options) === null || _f === undefined ? undefined : _f.document));
    }
    if (this.options.dropcursor !== false) {
      extensions.push(Dropcursor.configure((_g = this.options) === null || _g === undefined ? undefined : _g.dropcursor));
    }
    if (this.options.gapcursor !== false) {
      extensions.push(Gapcursor.configure((_h = this.options) === null || _h === undefined ? undefined : _h.gapcursor));
    }
    if (this.options.hardBreak !== false) {
      extensions.push(HardBreak.configure((_j = this.options) === null || _j === undefined ? undefined : _j.hardBreak));
    }
    if (this.options.heading !== false) {
      extensions.push(Heading.configure((_k = this.options) === null || _k === undefined ? undefined : _k.heading));
    }
    if (this.options.history !== false) {
      extensions.push(History.configure((_l = this.options) === null || _l === undefined ? undefined : _l.history));
    }
    if (this.options.horizontalRule !== false) {
      extensions.push(HorizontalRule.configure((_m = this.options) === null || _m === undefined ? undefined : _m.horizontalRule));
    }
    if (this.options.italic !== false) {
      extensions.push(Italic.configure((_o = this.options) === null || _o === undefined ? undefined : _o.italic));
    }
    if (this.options.listItem !== false) {
      extensions.push(ListItem.configure((_p = this.options) === null || _p === undefined ? undefined : _p.listItem));
    }
    if (this.options.orderedList !== false) {
      extensions.push(OrderedList.configure((_q = this.options) === null || _q === undefined ? undefined : _q.orderedList));
    }
    if (this.options.paragraph !== false) {
      extensions.push(Paragraph.configure((_r = this.options) === null || _r === undefined ? undefined : _r.paragraph));
    }
    if (this.options.strike !== false) {
      extensions.push(Strike.configure((_s = this.options) === null || _s === undefined ? undefined : _s.strike));
    }
    if (this.options.text !== false) {
      extensions.push(Text.configure((_t = this.options) === null || _t === undefined ? undefined : _t.text));
    }
    return extensions;
  }
});
...
var latexPlaceHolder = (latex) => {
  return latex.replace(/\\([a-zA-Z]+)/g, "_tex.bs.$1");
};
var parseLatexPlaceHolder = (placeholder) => placeholder.replace(/_tex\.bs\.(\w+)/g, "\\$1");
var CenteredLabeledImage = "display:flex;flex-direction:column;align-items:center;";

// src/tiptap-extensions/image.ts
var LabeledImage = Node2.create({
  name: "labeledImage",
  group: "block",
  content: "inline*",
  defining: true,
  ...
  addCommands() {
    return {
      insertLabeledImage: (attrs) => ({ commands: commands2 }) => {
      ...
      setLabeledImage: (attrs) => ({ commands: commands2 }) => {
        return commands2.setNodeMarkup(this.name, attrs);
      },
      deleteLabeledImage: () => ({ commands: commands2 }) => {
        return commands2.deleteNode();
      }
    };
  }
});

// src/converter/tex-variable.ts
class LatexVariable {
  static variables = {};
  static get(key) {
  ...
  static setLatexVars(variables) {
    const pattern = /\\var\{(\\[^{}]+)\}\{([^{}]+)\}/g;
    const matches2 = [...variables.matchAll(pattern)];
    return matches2.reduce((acc, match) => {
  ...
  static convertToLatex() {
    return Object.entries(LatexVariable.variables).map(([key, value]) => `\\var{${key}}{${value}}`).join(`
`);
  }
}

// src/converter/additional.ts
class BaseConverter {
  _call(method, args) {
...
class Begin extends BaseConverter {
  figure() {
    return "<figure>";
  }
  enumerate() {
...
class End extends BaseConverter {
  figure() {
    return "</figure>";
  }
  enumerate() {
...
class BeginEndConverter extends BaseConverter {
  toKatex(match) {
...
// src/converter/core.ts
var convertLatexToHtml = (latex) => {
  ...
  return latex.replace(variablePattern, (match, varname, ...others) => {
    ...
    const imgWidth = figureContent ? figureContent[1].replace(`=`, (match2) => `:`) : "";
    ...
    return `<figure ${centered ? CenteredLabeledImage : ""} ${label ? `id="${label[1]}"` : ""}>
        ...
        `;
  }).replace(BracketBracesPattern, (match, command2, bracket, braces, num, raw, ...others) => {
    const converter = LatexConverter.texToHtml.bracketbraces[command2];
    if (converter) {
      return converter({ braces, bracket });
    }
    return match;
  }).replace(l, (match, command2, content, text, num, raw, ...others) => {
    const converter = LatexConverter.texToHtml.bracesbracket[command2];
    if (converter) {
      const attrs = {
        color: content || "",
        src: content || "",
    ...
    return match;
  }).replace(latexPattern, (match, command2, content, text, num, raw, ...others) => {
    const converter = LatexConverter.texToHtml.bracesbracket[command2];
    if (converter) {
      const attrs = {
        color: content || "",
    ...
    return match;
  }).replace(beginEnd, (match, command2, content, ...others) => {
    return new BeginEndConverter()._call(command2, {
      command: command2,
      content,
      match
    });
...
var LatexConverter = {
  htmlToTex: {
    ...
    span: (attrs, content) => attrs.style?.includes("color") ? `\\textcolor{${getColor(attrs.style)}}{${content}}` : content,
    br: () => `\\\\`,
...
`;
  ...
  texToHtml: {
    ...
    bracesbracket: {
      ...
      chapter: ({ content }) => `<h1 >${convertLatexToHtml(content)}</h1>`,
      section: ({ content }) => `<h2 >${convertLatexToHtml(content)}</h2>`,
      subsection: ({ content }) => `<h3>${convertLatexToHtml(content)}</h3>`,
      subsubsection: ({ content }) => `<h4>${convertLatexToHtml(content)}</h4>`,
      paragraph: ({ content }) => `<h5>${content}</h5>`,
      subparagraph: ({ content }) => `<h6>${content}</h6>`,
      textbf: ({ content }) => `<b>${content}</b>`,
      textit: ({ content }) => `<i>${content}</i>`,
      underline: ({ content }) => `<u>${content}</u>`,
      par: ({ text }) => `<p>${text}</p>`,
      textcolor: ({ content, color }) => `<span style="color:${color}">${content}</span>`,
      item: ({ text }) => `<li>${convertLatexToHtml(text)}</li>`,
      includegraphics: ({ content }) => `<img src="${content}"  />`,
      href: ({ content }) => `<a href="">${content}</a>`,
      rule: () => `<hr />`,
      quote: ({ content }) => `<blockquote>${content}</blockquote>`,
      texttt: ({ content }) => `<code>${content}</code>`,
      verbatim: ({ content }) => `<pre>${content}</pre>`,
      sup: ({ content }) => `<sup>${content}</sup>`,
      sub: ({ content }) => `<sub>${content}</sub>`,
      footnotesize: (content) => `<small>${content}</small>`,
      begin: (opt) => {
        const content = opt.content;
        const begin = new Begin;
        return begin._call(content, opt);
      },
      end: (opt) => {
        const end = new End;
        return end._call(opt.content, opt);
      },
      sqrt: ({ match }) => {
...
var getColor = (style2) => {
  const match = style2.match(/color:\s*([^;]+)/);
  return match ? match[1] : "black";
};
// src/tiptap-extensions/var.ts
var TexVarExtension = Node2.create({
  name: "var",
  group: "inline",
  inline: true,
  atom: true,
  ...
  renderHTML({ node }) {
    ...
    return [
      ...
      LatexVariable.get(node.attrs.varname) ?? ""
    ];
  },
  addCommands() {
    return {
      insertTexVar: (varname) => ({ commands: commands2 }) => {
        return commands2.insertContent({
          type: this.name,
          attrs: { varname }
        });
...
var htmlToTex = convertLatexToHtml(`
